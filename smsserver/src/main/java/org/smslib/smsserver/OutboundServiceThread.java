...
public class OutboundServiceThread extends Thread
{
	...
	{
		if (Service.getInstance().getGroupManager().exist(message.getRecipient().getNumber()))
		{
			...
			int counter = 0;
			for (MsIsdn recipient : Service.getInstance().getGroupManager().getGroup(message.getRecipient().getNumber()).getRecipients())
			{
				counter++;
				...
				s.setString(3, (message.getOriginator() != null && message.getOriginator().getType() != MsIsdn.Type.Void ? message.getOriginator().getNumber() : ""));
				s.setString(4, recipient.getNumber());
